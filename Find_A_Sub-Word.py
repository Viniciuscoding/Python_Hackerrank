...
We define a word to be a contiguous sequence of one or more word characters that is preceded and succeeded by one or more occurrences of 
non-word-characters or line terminators. For example, in the string I l0ve-cheese_?, the words are I, l0ve, and cheese_.

We define a sub-word as follows:
1. A sequence of word characters (i.e., English alphabetic letters, digits, and/or underscores) that occur in the same exact order 
...
Each line i of q the  subsequent lines contains a string, si, to check.
Constraints
1 <= n <= 100
1 <= q <=10
...
While s is a substring of the word "this", it's followed by a blank space; because a blank space is non-alphabetic, non-numeric, and not an underscore, 
...
q = int(input())
for _ in range(q):
    print(len(re.findall(r'(?<=\w)(?=%s\w)' % input().strip(), sentence)))
