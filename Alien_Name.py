...
4. It may be terminated with an optional "_" (ASCII value 95). Note that if it's not terminated with an underscore, then there should be no characters after the 
sequence of 0 or more English letters.
...
Output Format
Iterate through each of the n strings in order and determine whether or not each string is a valid alien username. If a username is a valid alien username, 
...
for _ in range(int(input())):
    if re.match(r'^[_.]\d+[a-zA-Z]*_?$', input()):
        print("VALID")
    else:
        print("INVALID")

# ONE-LINER SOLUTION
print('\n'.join(["VALID" if re.match(r'^[_.]\d+[a-zA-Z]*_?$', input()) else "INVALID" for _ in range(int(input()))]))
